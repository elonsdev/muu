...
contract EnsMapper is Ownable, ERC721A {
    ...
    mapping (uint256 => Muu) private muus;
    ...
    string public domainLabel = unicode"finalcrootest";
    ...
    constructor() ERC721A("MUU", "muu.eth") {
    ...
    function mint(string calldata label) public payable {   
        ...
        uint256 seed1 = uint256(keccak256(abi.encodePacked(block.difficulty, token_id)));
        ...
        Muu memory newMuu = Muu(
                Base64.toString(seed1 % 1000000),    
                Base64.toString((seed1 % 6) + 1),
                Base64.toString((seed1 % 9) + 1),
                Base64.toString((seed1 % 5) + 1),
                (seed1 % 18) + 1,
                (seed1 % 30) + 1,
                Base64.toString((seed1 % 4) + 1),
                (seed1 % 16) + 1
        );
        
        muus[token_id] = newMuu;
        _mint(msg.sender, 1);
   ...
   function getTokenDomain(uint256 token_id) private view returns(string memory uri){
    ...
    function buildImage(uint _tokenId) public view returns(string memory) {
        Muu memory currentMuu = muus[_tokenId];

        string memory above10Mouth = "";
        string memory above10Top = "";
        string memory above10Eyes = "";

        if (currentMuu.mouth < 10) {
            above10Mouth = "0"; 
        }

        if (currentMuu.top < 10) {
            above10Top = "0"; 
        }

        if (currentMuu.eyes < 10) {
            above10Eyes = "0"; 
        }

        string memory result = string(abi.encodePacked(
            ':',currentMuu.seed,'.svg',       
            '?face=variant0',currentMuu.face,
            '&nose=variant0',currentMuu.nose,
            '&beard=variant0',currentMuu.beard,
            '&mustache=variant0',currentMuu.mustache,
            '&mouth=variant',above10Mouth,Base64.toString(currentMuu.mouth),
            '&top=variant',above10Top,Base64.toString(currentMuu.top),
            '&eyes=variant',above10Eyes,Base64.toString(currentMuu.eyes)
            ));
    ...
    function setbaseApiUri(uint256 _price) public onlyOwner {
        price = _price;
    }
